...
def load_model_results(artifacts_dir: Path) -> dict:
    ...
    for model_dir in artifacts_dir.iterdir():
        ...
        try:
            ...
            results[model_name] = {"model": model, "path": model_dir}
...
def create_title_page(pdf, title="Model Evaluation Report"):
    ...
    ax.text(0.5, 0.7, title, ha="center", va="center", fontsize=28, fontweight="bold")
    ax.text(
        0.5, 0.6, "Crypto Volatility Detection", ha="center", va="center", fontsize=18
    )
    ax.text(
        0.5,
        0.5,
    ...
    for model_name, data in models_dict.items():
        ...
        if hasattr(model, "predict"):
            ...
            if model_name == "baseline":
                ...
                available_features = [
                    f for f in baseline_features if f in X_test.columns
                ]
...
def main():
    ...
    parser.add_argument(
        "--output", default="reports/model_eval.pdf", help="Output PDF path"
    )
