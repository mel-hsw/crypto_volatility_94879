...
def compare_outputs(replay_df: pd.DataFrame, original_file: str, compare_labels: bool = False):
    """
    ...
    if not compare_labels:
        label_cols = {'volatility_spike', 'label', 'future_volatility', 'price_pct_change', 'chunk_id', 'time_diff'}
    ...
    feature_cols = [col for col in replay_df.columns 
                   if col not in ['volatility_spike', 'label', 'future_volatility', 'price_pct_change', 
                                 'chunk_id', 'time_diff'] and 
                   pd.api.types.is_numeric_dtype(replay_df[col])]
    ...
    if len(replay_df) == len(original_df):
        ...
        if compare_labels and 'volatility_spike' in replay_df.columns and 'volatility_spike' in original_df.columns:
            label_match = (replay_df['volatility_spike'] == original_df['volatility_spike']).sum()
            label_match_pct = label_match / len(replay_df) * 100
            logger.info(f"\n  Label comparison:")
            logger.info(f"    volatility_spike match: {label_match_pct:.2f}% ({label_match}/{len(replay_df)})")
    else:
...
def main():
    ...
    parser.add_argument('--add-labels', action='store_true', default=False,
    ...
    parser.add_argument('--compare-labels', action='store_true', default=False,
    ...
    if args.add_labels:
        ...
        if 'volatility_spike' in features_df.columns:
            ...
            logger.info(f"  Spike rate: {spike_rate:.2%} ({spike_count} spikes out of {len(features_df)} total)")
