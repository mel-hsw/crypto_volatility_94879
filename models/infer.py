...
class VolatilityPredictor:
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'throughput_per_second': (self.prediction_count / self.total_inference_time
...
def benchmark_inference(model_path: str, features_path: str, n_samples: int = 1000):
    ...
    if batch_time < max_allowed_time:
        print(f"✓ PASSED: Inference is {max_allowed_time / batch_time:.1f}x faster than requirement")
    else:
        print(f"✗ FAILED: Inference is too slow ({batch_time / max_allowed_time:.1f}x requirement)")
...
def run_live_inference(model_path: str, features_path: str):
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        ...
        print(f"{timestamp} {result['prediction']:<12} {result['probability']:<12.3f} "
...
def main():
    parser = argparse.ArgumentParser(description="Run inference for volatility detection")
